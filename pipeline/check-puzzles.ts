...
for (const [date, entry] of Object.entries(schedule.days)) {
  ...
  if (tier === undefined) {
    problems.push(
      `${date}: scheduled answer "${entry.name}" (${entry.id}) vanished from the snapshot`,
    );
  } else if (tier !== 1) {
    problems.push(
      `${date}: scheduled answer "${entry.name}" (${entry.id}) dropped to tier ${tier}`,
    );
  }
}
...
if (problems.length > 0) {
  console.error(
    `PUZZLE INTEGRITY: ${problems.length} problem(s) in the next ${HORIZON_DAYS} days:`,
  );
  for (const p of problems) console.error("  " + p);
  console.error(
    "Fix: edit puzzles/daily.json (swap the affected days), then `tsx puzzles/generate-daily.ts --emit-only`.",
  );
  process.exit(1);
}
